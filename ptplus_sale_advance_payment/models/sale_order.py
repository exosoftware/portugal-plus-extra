...
class SaleOrder(models.Model):
    ...
    def _compute_advance_payment(self):
        ...
        the company down payment account (``company_id.downpayment_account_id``),
        which by definition is *never* a receivable account. As a result those
        payments are invisible to the base computation and ``amount_residual``
        is never reduced.
        ...
        for order in self:
            downpayment_acc = order.company_id.downpayment_account_id
